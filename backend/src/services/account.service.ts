...
export async function addCreditExpense(
  ...
  const details_data = await conn.query(
    `INSERT INTO despesas_credito(dt_despesa_credito, despesa_credito, valor, descricao, idconta) VALUES ("${formattedDate()}", "${creditExpense}", ${creditExpenseAmount}, "${description}", ${accountId})`
  ).then(data => data[0]);
